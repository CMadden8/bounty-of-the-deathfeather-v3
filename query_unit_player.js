// Query specific GameObject details via MCP
const WebSocket = require('C:/Users/madde/Bounty_Of_The_Deathfeather_V3/Library/PackageCache/com.gamelovers.mcp-unity@0d4643656803/Server~/node_modules/ws');

const ws = new WebSocket('ws://localhost:8090/McpUnity');

ws.on('open', () => {
    const payload = {
        id: 'select_unit',
        method: 'select_gameobject',
        params: {
            path: 'CombatPOC/GridController/UnitManager/Unit_Player'
        }
    };
    ws.send(JSON.stringify(payload));
    
    // Also query GameObject details after selection
    setTimeout(() => {
        const detailsPayload = {
            id: 'get_details',
            method: 'get_gameobject_details',
            params: {
                path: 'CombatPOC/GridController/UnitManager/Unit_Player'
            }
        };
        ws.send(JSON.stringify(detailsPayload));
    }, 500);
});

ws.on('message', (data) => {
    const response = JSON.parse(data.toString());
    console.log(JSON.stringify(response, null, 2));
});

ws.on('error', (error) => {
    console.error('MCP Unity server error:', error.message);
    process.exit(1);
});

setTimeout(() => ws.close(), 2000);
